// hive-tick v4 — met Realtime Broadcast events
// Deploy via: supabase functions deploy hive-tick
//
// Broadcast payload structuur:
//   { soort: "zorg",        naam, label, kleur, animeer, tekst }
//   { soort: "bezoek",      naamA, naamB, tekst }
//   { soort: "kweek-start", naamA, naamB, tekst }
//   { soort: "geboren",     naamKind, generatie, tekst }
...
const ZORG_PER_TICK     = 3;
const MAX_CATCHUP_TICKS = 5000;

const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);

// -------- helpers (identiek aan index.html) --------
...
function zorg(b: any) {
  for (const fn of ZORGACTIES) { const r = fn(b); if (r) return r; }
...
function maakKind(ouderA: any, ouderB: any, namen: string[]) {
  ...
  return maakBotty(naam, { ...palet, naam: palet.naam || "mix" }, gen, { datakwaliteit:dk, efficientie:ef, mutaties });
}
...
async function broadcast(payload: object) {
  // Gebruik fetch direct naar Supabase Realtime REST broadcast endpoint
  // Supabase broadcast via de JS client werkt ook maar vereist een subscribed channel.
  // Hier gebruiken we de REST API zodat de edge function geen open WS nodig heeft.
  try {
...
Deno.serve(async () => {
  ...
  const events: object[] = [];   // verzamel events om te broadcasten

  // catch-up ticks (geen broadcast — niemand kijkt als we inhalen)
  for (let t = 0; t < gemist - 1; t++) {
    ...
    bottys.forEach(b => { const s = huidigeStage(b); if (s !== b.stage) b.stage = s; });
  }

  // live tick (broadcast events)
  if (gemist >= 1) {
    ...
    if (Math.random() < 0.05) {
      ...
      if (kandidaten.length >= 2) {
        ...
        // Vervang ouderA door kind (met korte vertraging gesimuleerd door bezigEi)
  ...
  // Broadcast events (best-effort, na opslaan)
  for (const ev of events) await broadcast(ev);

  return new Response(JSON.stringify({ ok:true, ticks: gemist, events: events.length }), {
    headers: { "Content-Type": "application/json" },
  });
});

// hulpfunctie voor floater-data bijhouden in tick (niet visueel, alleen state)
function floater_data(_b: any, _label: string, _kleur: string) { /* no-op in edge function */ }
